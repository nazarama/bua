...
export default reactive({
  ...
  event: {club: null, desc: null, name: null, flyer: null, fb:null},
  ...
  updateQuery(newClub: any, newDate: any | null) {
    ...
    if (newDate && newDate.value && newDate.value[0]) { this.query.from = newDate.value[0];  } else  {this.query.from = null} 
    if (newDate && newDate.value && newDate.value[1]) { this.query.to = newDate.value[1]; } else  {this.query.to = null} 
      ...
      for(var i = 0; i < pom.length; i++)
        {   
            let pomImage: Image = { src: "", caption: "", date: null };
              pomImage.src =  "data:image/jpg;base64," + `${(await axios.get(`/api/image/${pom[i].topImage[0]}`)).data.data}`;
  ...
  async fetchPodcastsPage(skip: string, limit: string ) {
    try {
      this.resultForPodcastsPage = (
        await axios.get<[]>(`/api/podcast/page/${skip}/${limit}`)
      ).data;
  } catch (e) {
    console.error("error", e)
  }
...
runFetchPodcastPage(skip: string, limit: string) {
  ...
  async fetchPodcast(order: string) {
    try {
      this.resultForPodcast = (
        await axios.get<[]>(`/api/podcast/${order}`)
      ).data;
      ...
      this.podcast.podcastImage = "data:image/jpg;base64," + `${(await axios.get(`/api/image/${this.resultForPodcast.podcastimage}`)).data.data}`;;
  ...
  async fetchEvents() {
    try { 
      ...
      this.resultForMap = (
        await axios.get<[]>("/api/club/filter", { params })
      ).data;
      
      if (this.resultForMap.length === 0) {
        console.log("result ===0", this.resultForMap.length )
      ...
      const pom2 = pom.map((element: any)=>({lat: element.position.lat, lng: element.position.lng}));
      this.center = this.findCenter(pom2);
      console.log(this.center);
      } else {
        this.center = { lat: 48.136933314107374, lng: 17.115584408777007 }
      }
      this.resultForEvents = (
        await axios.get<[]>("/api/club/flatten", { params })
      ).data;
      // }
    } catch (e) {
      console.error("error", e);
  ...
  async fetchEvent(clubId: string, eventId: string) {
    try {
      this.resultForEvent = (
        await axios.get<[]>(`/api/club/${clubId}/${eventId}`)
      ).data;
      ...
      this.event.flyer = "data:image/jpg;base64," + `${(await axios.get(`/api/image/${this.resultForEvent[0].event[0].image}`)).data.data}`;;
    } catch (e) {
      console.error("error", e);
    }
  },
