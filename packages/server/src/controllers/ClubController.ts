import {
  ...
  Inject,
  PathParams,
  QueryParams, Controller, Get, Post, Put
} from "@tsed/common";
...
export class ClubController {
  ...
  @Get("/clubs")
  async listClubs() {
    return this.clubModel.find({}).select("name");
  }
  @Get("/filter")
  async listBy(
    @QueryParams("from") from: Date,
    ...
    if (!from && club) {
      ...
      if (club) {
        const result = await this.clubModel
          .find({ event: { $exists: true }, name: club })
          .populate({
            path: "event",
            options: { sort: { "from": 1 } },
  ...
  @Put("/")
  async save(
    @BodyParams("club") club: ClubModel
  ): Promise<MongooseModel<ClubModel>> {
 ...
 */
  @Get("/:clubId/:eventId")
  async findEvent(
    @Required @PathParams("clubId") clubId: string,
    @Required @PathParams("eventId") eventId: string
  ) {
    const result = await this.clubModel
      .find({ event: { $exists: true }, _id: clubId })
      .populate({
        path: "event",
