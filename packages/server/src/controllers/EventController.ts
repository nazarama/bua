...
export class EventController {
  ...
  // @Get("/:clubId/:from/:to")
  public async listBy(
    @Required() @QueryParams("from") from: Date,
    @Required() @QueryParams("to") to: Date
  ) {
    const result = await this.model
      .find({ from: { $gte: from, $lte: to } })
      .exec();
    return result;
  }
  ...
  @Post("/")
  async save(
    @BodyParams("event") event: EventModel
  ): Promise<MongooseModel<EventModel>> {
